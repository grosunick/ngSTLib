...
namespace ng::timer
{
    ...
    template<typename TIM, TimChannel channel = TimChannel::ch1> class TimGeneral : public TimBase<TIM> {
        template<typename OCxM, OutCaptCmpMode OCM> __force_inline void setOCxM() {
            if constexpr (OCM == OutCaptCmpMode::ActiveLevelOnMatch) {
                OCxM::ActiveLevelOnMatch::set();
            } else if constexpr (OCM == OutCaptCmpMode::InactiveLevelOnMatch) {
                OCxM::InactiveLevelOnMatch::set();
            } else if constexpr (OCM == OutCaptCmpMode::Toggle) {
                OCxM::Toggle::set();
            } else if constexpr (OCM == OutCaptCmpMode::ForceInactive) {
                OCxM::ForceInactive::set();
            } else if constexpr (OCM == OutCaptCmpMode::ForceActive) {
                OCxM::ForceActive::set();
            } else if constexpr (OCM == OutCaptCmpMode::PWM1) {
                OCxM::PWM1::set();
            } else if constexpr (OCM == OutCaptCmpMode::PWM2) {
                OCxM::PWM2::set();
            }
        }
